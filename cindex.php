...
<head>
  ...
  <?php $note ="";
    ...
    { 
      ...
      $note = "<div class='alert alert-info'>successfully transaction done</div>";
      else
...
<body style="background: url(images/cas.png); background-size: 100% ;font-family: Times New Roman" class="bg-gradient-seconday">
           ...
           $row2 = $array3->fetch_assoc();
            ...
            <div class='row'>
                  ...
                  <div class='col'>
                    ...
                    <input type='hidden' value='$row2[userId]' name='userId' class='form-control ' required>
                    <input type='number' class='form-control my-1' name='checkno' placeholder='Write Check Number' required>
                    <input type='number' class='form-control my-1' name='amount' placeholder='Write Amount for withdraw' max='$row2[balance]' required>
                   <button type='submit' name='withdraw' class='btn btn-info btn-bloc btn-sm my-1'> Withdraw</button></form><form method='POST'> 
                    <input type='hidden' value='$row2[accountNo]' name='accountNo' class='form-control ' required>
                    <input type='hidden' value='$row2[userId]' name='userId' class='form-control ' required>
                   <input type='number' class='form-control my-1' name='checkno' placeholder='Write Check Number' required>
            ...
            echo "<div class='alert alert-info w-50 mx-auto'>Account No. $_POST[otherNo] Does not exist</div>";
        }
  } 
      ?>
  </div>
