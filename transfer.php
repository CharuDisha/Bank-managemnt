...
<div class="container">
  ...
  <div class="card-body alert alert-dark" >
      <form method="POST">
          <div class="alert alert-dark w-75 mx-auto">
            <h5>New Transfer</h5>
            <input type="text" name="otherNo" class="form-control " placeholder="Enter Receiver Account number" required>
            <button type="submit" name="get" class="btn btn-info btn-bloc btn-sm my-1">Get Account Info</button>
          </div>
      </form>
          ...
          { $row2 = $array2->fetch_assoc();
            echo "<div class='alert alert-dark w-50 mx-auto'>
                  <form method='POST'>
                    ...
                    <button type='submit' name='transfer' class='btn btn-info btn-bloc btn-sm my-1'>Transfer</button>
                  </form>
                </div>";
           ...
           $row2 = $array3->fetch_assoc();
            echo "<div class='alert alert-dark w-50 mx-auto'>
                  <form method='POST'>
                    ...
                    Enter Amount for transfer.
                    <input type='number' name='amount' class='form-control' min='1' max='$userData[balance]' required>
                    <button type='submit' name='transferSelf' class='btn btn-info btn-bloc btn-sm my-1'>Transfer</button>
                  </form>
                </div>";
          }
          else
    ...
    if (isset($_POST['transfer']))
    {
      ...
      $array = $con->query("select * from transaction where userId = '$userData[userId]' AND action = 'transfer' order by date desc");
      if ($array ->num_rows > 0) 
      {
         while ($row = $array->fetch_assoc()) 
         {
            ...
            {
              echo "<div class='list-group-item list-group-item-action bg-gradient-info'>$row[date] Transfer of Rs.$row[debit] has been made to account no.$row[other]</div>";
            }
  ...
  <div class="card-footer alert alert-dark text-center text-muted">
   <?php echo bankname ?>
  </div>
</div>
